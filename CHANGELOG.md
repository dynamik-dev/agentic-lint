...
- Trust gate now fires inside `_cmd_session_record` and `_cmd_stop`. Previously, an untrusted `.bully.yml` reaching the Stop hook would parse rules, evaluate session-rule violations, write `.bully/session.jsonl`, and could return exit 2 — a config the user had not yet reviewed could block their session-end. The PostToolUse path was already gated; the session-rule path was not. Now both subcommands check `_trust_status()` before any rule work: `_cmd_session_record` returns 0 silently (its hook caller wraps in try/except: pass), `_cmd_stop` emits the standard untrusted stderr message and returns 0. Regression coverage in `pipeline/tests/test_trust_gate_stop.py`.
